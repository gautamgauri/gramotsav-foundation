# Blind School Project — Funder One-Pager

*Gramotsav Foundation · Patna, Bihar · "Mitti Se Manch Tak"*

---

## The problem

Blind and visually impaired students in Bihar face compounding barriers. Conventional textbooks, digital content and classroom resources are not built for them. Braille, audio and screen-reader compatible material is limited or expensive. And even where academic access improves, students still lack the mobility, communication, digital and independent-living skills that decide whether education translates into a life.

Meanwhile, the things that build confidence and belonging — music, theatre, sport, cultural participation — are treated as optional extras rather than as part of how a child learns to be present in the world.

## What we propose

An inclusive education and development model delivered inside an existing blind or special school, combining six components around the same student:

| | Component | What it covers |
|---|---|---|
| **A** | Accessible education | Braille and tactile materials, audio lessons, accessible worksheets and exams, individualised support |
| **B** | Digital & assistive technology | Computer and smartphone literacy, screen readers, Braille displays, safe internet use |
| **C** | Life skills & independence | Daily living, communication, orientation and mobility, practical financial and digital skills |
| **D** | Music, arts & culture | Singing and instruments, rhythm and voice, theatre and storytelling, cultural performance |
| **E** | Sports & well-being | Accessible physical activity, yoga, adapted games, peer interaction and mental wellbeing |
| **F** | Career & livelihood | Career awareness, vocational skills, professional mentorship, employment and enterprise exposure |

## Why us

We have already run and closed an audio-first music programme for children with visual impairment, and **published its full impact and closure report** — including what did not work. That report is public on our website.

Our teaching method does not depend on Braille notation. It is built on listening, audio demonstration, spoken instruction, repetition, rhythmic cues, musical memory and hands-on practice — which is why it reaches students that notation-based music teaching cannot.

## How it runs

Eight phases across the first year: **need assessment → baseline → resource development → training → regular programme → mentoring and exposure → monitoring → scale and sustainability.**

We embed into an existing institution first. That gets resources to students quickly, builds the capacity of teachers already in the room, and avoids a parallel system that collapses when the funding ends.

## How you will know it worked

- Students enrolled and regularly participating; attendance and retention
- Improvement in academic and digital skills
- **Number of students using assistive technology independently**
- Participation in music, arts, sports and cultural activity
- Life-skill and confidence assessments
- Parent and teacher feedback; student portfolios and documented achievements
- External exposure, mentorship and career opportunities created

## What we are asking for

A **multi-year CSR or institutional partnership** covering one site end to end. Budget heads: learning materials, assistive technology, human resources, music and arts, accessibility infrastructure, student activities, monitoring and documentation, and administration.

**A costed line-item budget is prepared per site** and sent on request — cost depends on student numbers, the host institution's existing facilities, and the assistive-technology specification we agree with you.

## What you get back

Named multi-year partnership with defined milestones · quarterly narrative and financial reporting in your format · employee volunteering and mentorship opportunities · a published closure report · and a documented model your CSR team can point to and another institution can adopt.

## Theory of change

> If blind and visually impaired students receive accessible education, appropriate technology, life skills, creative opportunities, mentorship and a supportive environment, then their confidence, independence, learning outcomes and participation can improve — contributing to greater educational, social and livelihood opportunity.

## Risks we have named up front

| Risk | Mitigation |
|---|---|
| Limited resources | Phased implementation, diversified funding |
| Low technology familiarity | Hands-on training, ongoing technical support |
| Irregular participation | Family engagement, mentoring, student-centred activities |
| Lack of trained staff | Capacity building, specialist partnerships |
| Sustainability after funding | Institutional integration, multi-year partnerships |

---

**Gramotsav Foundation** · Section 8 non-profit company · CIN U88900BR2026NPL083562
Provisional tax registration and donor-deduction approval granted 24 August 2026
gramotsavfoundation@gmail.com · @gramotsavfoundation · Patna, Bihar

> **Every child deserves a chance to learn, grow and perform.**
